...
module.exports = {
  ...
  returnMockData: function returnMockData(mockData, res){
    // TODO: need to be improved the code quality
    ...
    if(body){
      ...
      // recalculate the body length to make sure the content won't be truncated.
    ...
    mkdirp(path.dirname(dir), function (err) {
        if ("mkdirp function error", err) {
          ...
          fs.writeFileSync(dir, jsonFormat(obj, formatter), function(err){
            if(err) {
              return console.log("writeFileSync error", err);
            }
  ...
  updateConfig: function updateConfig(dir, route, cache){
    if(!cache.routeTable.includes(route)){
      cache.routeTable.push(route);
      let config_dir = path.join(dir, 'config.js');
      cache.config = cache.config.replace(/(routes\s*:\s*\[[\s\S]*?)(\])/, "$1" + '//' + JSON.stringify(route) + ',\n]');
      let formatData = beautify(cache.config, { indent_size: 2 });
