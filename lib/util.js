...
module.exports = {
  prepareConfig: function prepareConfig(options){
    ...
    let dir = path.join(options.dir.replace(path.resolve()), 'config.js');
    console.log('modified path is ', options.dir.replace(path.resolve()));
    console.log("config dir is ", dir);
    let config = {};
  ...
  getNameByReq: function getNameByReq(req, distinguisher){
    ...
    let ext = '.json';
    if(distinguisher.length != 0){
      let reqbody = module.exports.getReqBody(req);
