...
const defaultsKeysDir = path.join(__dirname, '..', 'keys');
const filter = ['xts'];
...
module.exports = class Server {
    ...
    start() {
      ...
      // https -> https set ssl set secure: false;
        let self = this;
        let protocal = self.https ? https : http;
        let {proxyOpt, protocalOpt} = this.resolveProtocalOpt();
        proxy =
        server = https.createServer(protocalOpt, function (req, res) {
            let body = [];
            req.on('error', (err) => {
                console.error(err);
            }).on('data', (chunk) => {
    ...
    resolveProtocalOpt(){
      let keyDir = this.keyDir || defaultsKeysDir;
      let proxyOpt, protocalOpt;
      if(this.https){
        proxyOpt = {
