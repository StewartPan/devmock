  ...
  start(){
      ...
      self.server = http.createServer(function(req, res){
        ...
        req.on('error', (err) => {
          ...
          let route, ok;
          [route, ok] = getRouteByReq(req, body, self.distinguisher);
          if(ok && self.routesInUse.includes(route)){
      ...
      proxy.on('proxyRes', function (proxyRes, req, res) {
        if(isTargetReq(req, self.target_url)){
          ...
          let [route, ok] = getRouteByReq(req, body, self.distinguisher);
          if(ok){
            if(self.recordLatest || self.configString.indexOf(JSON.stringify(route)) < 0){
              ...
              proxyRes.on('end', function () {
                ...
                // if(route === "POST\\MstrMainBranch\\servlet\\taskProc\\getServerJSONResults.json"){
                //   console.log(Resbody);
                // }
                try{
