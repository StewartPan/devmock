...
const {isTargetReq, readConfig, recordMockData, writeConfig, decompressData, compressData, streamifyStr, addRoute} = require('./util');
const filter = ['xts', 'mstrWeb'];
      ...
      // https -> https set ssl, set secure: false;
        ...
        function serverHandler(req, res){
            ...
            req.on('error', (err) => {
                ...
                let routePath = findRoutePath(self.routes, route);
                if (routePath !== null) {
                    if(isUrl(routePath)){
                        proxy.web(req, res, {target: routePath, buffer: bufferStream});
                    }else {
...
function getRouteByReq(req, mstrMode) {
    ...
    if (mstrMode) {
        ...
        let route = path.join(req.method, url.parse(req.url).pathname);
