      ...
      if(isTargetReq(req, target_url)){
        ...
        if(ok){
          ...
          proxyRes.on('end', function () {
            ...
            // TODO: unable to parse some big string eg: length > 150000
            // may use stream to solve this problem.
            try{
            ...
            // if(proxyRes.headers.hasOwnProperty('content-type') && proxyRes.headers['content-type'].split(';')[0] === 'application/json' && Resbody.trim().length != 0){
            //   data.body = Resbody;
            // }else{
            //   data.body = Resbody;
            // }
